<section class="<?php echo((get_field('margin')) ? " md:my-10 my-12 " : "") ?> <?php echo get_field('background-color') ?> <?php echo get_field('padding') ?>">
    <div class="container mb-[-70px] grid grid-cols-2 gap-3">
        <div class="col-span-2 md:col-span-1">
            <h2 class="text-55 leading-55 font-bold text-black mb-3"><?php echo get_field('title'); ?></h2>
            <h3 class="test-22 leading-32 font-medium text-roze uppercase"><?php echo get_field('subtitle'); ?></h3>
        </div>
        <?php 
        $link = get_field('button');
        if( $link || echo get_field('text__button')): 
            $link_url = $link['url'];
        ...
        <?php 
        if( !echo get_field('text__button')): ?>
